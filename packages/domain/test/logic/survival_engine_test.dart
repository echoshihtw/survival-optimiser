  ...
  group('computeModel', () {
    test('returns model with subscription cost even for no months', () {
      final m = computeModel(
        months: [], monthlyPayment: 0, subscriptionMonthlyCost: 5000);
    ...
    test('currentCash is last balance', () {
      ...
      final m = computeModel(
        months: months, monthlyPayment: 0, subscriptionMonthlyCost: 0);
    ...
    test('burnRate uses grossOutflow not netFlow', () {
      final months = [
        MonthlyState(month: SurvivalMonth(DateTime(2025, 1)),
            netFlow: 30000, balance: 1030000, grossOutflow: 50000),
        MonthlyState(month: SurvivalMonth(DateTime(2025, 2)),
            netFlow: -10000, balance: 1020000, grossOutflow: 60000),
      ];
      final m = computeModel(
        months: months, monthlyPayment: 0, subscriptionMonthlyCost: 0);
    ...
    test('burnRate is 0 when no gross outflows', () {
      final months = [
        MonthlyState(month: SurvivalMonth(DateTime(2025, 1)),
            netFlow: 80000, balance: 1080000, grossOutflow: 0),
      ];
      final m = computeModel(
        months: months, monthlyPayment: 0, subscriptionMonthlyCost: 0);
      expect(m.burnRate, 0);
    ...
    test('runOutDate is null when burn rate is zero', () {
      final months = makeMonths(
          startBalance: 1000000, netFlowPerMonth: 0,
          count: 3, grossOutflowPerMonth: 0);
      final m = computeModel(
        months: months, monthlyPayment: 0, subscriptionMonthlyCost: 0);
    ...
    test('runway calculates mathematically beyond projection cap', () {
      ...
      final months = [
        MonthlyState(month: SurvivalMonth(DateTime(2025, 1)),
            netFlow: -167, balance: 300000, grossOutflow: 167),
      ];
      final m = computeModel(
        months: months, monthlyPayment: 0, subscriptionMonthlyCost: 0);
    ...
    test('subscription cost reduces runway', () {
      ...
      final withSub = computeModel(
        months: months, monthlyPayment: 0, subscriptionMonthlyCost: 10000);
    ...
    test('filledHearts clamps between 0 and 12', () {
      ...
      final m = computeModel(
        months: high, monthlyPayment: 0, subscriptionMonthlyCost: 0);
